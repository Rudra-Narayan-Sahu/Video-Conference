...
function HomeComponent() {
  ...
  return (
    <div className="app-container" style={{ minHeight: '100vh' }}>
      ...
      <header className="navbar-wrapper">
        <nav className="navbar">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <Link to="/history" className="btn-ghost" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <Clock size={15} />
              <span>History</span>
            </Link>

            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'var(--bg-surface-elevated)', padding: '4px 12px', borderRadius: '6px', border: '1px solid var(--border-default)' }}>
              <span style={{ fontSize: '13px', fontWeight: 500, color: '#fff' }}>{user?.name || user?.username}</span>
            </div>

            <button onClick={handleLogout} className="btn-ghost" style={{ color: '#f87171' }} title="Logout">
              <LogOut size={16} />
            </button>
          </div>
        </nav>
      ...
      {/* Main Body */}
      <main className="section-container" style={{ maxWidth: '1000px', paddingTop: '40px' }}>
        {/* Welcome */}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '16px', marginBottom: '36px' }}>
          ...
          <div className="panel" style={{ padding: '24px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <div>
              ...
              <h3 style={{ fontSize: '17px', fontWeight: 600, color: '#fff', marginBottom: '4px' }}>Start a New Meeting</h3>
          ...
          <div className="panel" style={{ padding: '24px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <div>
              ...
              <h3 style={{ fontSize: '17px', fontWeight: 600, color: '#fff', marginBottom: '4px' }}>Join with a Code</h3>
              <p style={{ fontSize: '13px', color: 'var(--text-secondary)', lineHeight: 1.5 }}>
                Enter the room ID or meeting invitation link to connect.
              </p>
            </div>

            <form onSubmit={handleJoinMeeting} style={{ marginTop: '20px', display: 'flex', gap: '8px' }}>
              <input 
                type="text" 
